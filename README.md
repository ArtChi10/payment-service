...
Встроенный `PaymentGateway` — только эмуляция внешнего сервиса: он принимает и логирует
`gateway_operation_id`, но не хранит результат по ключу и не моделирует PSP-side
deduplication. В production это обязательная ответственность внешнего gateway/PSP API.
...
короткого retry-read окна после `IntegrityError`, либо вынести идемпотентность в отдельную
таблицу request keys. Для тестового задания текущий контракт покрыт concurrent POST test:
создается один payment и одно outbox event.
...
```bash
RUN_RABBITMQ_INTEGRATION=1 pytest -m integration
```
